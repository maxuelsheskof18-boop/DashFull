document.addEventListener('DOMContentLoaded', () => {
    ...
    window.atualizarQtdFeitaItem = async function(idEnvio, sku, valor) {
        const item = (((dadosLocais || []).find(x => String(x.id_envio) === String(idEnvio)) || {}).itens || {})[sku] || {};
        ...
        const conferido = declarado > 0 && qtdFeita >= declarado;
        const url = `${FIREBASE_BASE}/historico_envios/${idEnvio}/itens/${sku}.json`;
        await fetch(url, {
            method: 'PATCH',
    ...
    window.salvarObsItem = async function(idEnvio, sku, valor) {
        ...
        await fetch(url, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ observacao_item: (valor || '').trim(), atualizado_em: new Date().toLocaleString('pt-BR') })
        });
    };
...
window.reverterParaPendente = async function(idEnvio) {
  ...
  try {
    ...
    function itemBateBusca(item, query) {
        ...
        if (textosBase.includes(query)) return true;
        ...
        return Object.keys(itens).some(sku => {
            ...
            return composto.includes(query);
        });
    }
        ...
        envios.forEach(item => {
            ...
            if (listaItens) {
                ...
                itensHtml += `
                    ...
                    <div class="itens-grid-body">`;

                entriesItens.forEach(([sku, prod]) => {
                    prod = prod || {};
                    const declarado = Number(prod.declarado) || 0;
                    const qtdFeita = Number(prod.qtd_feita) || 0;
                    const statusCtrl = String(prod.status_controle || 'Pendente');
                    const observacaoItem = String(prod.observacao_item || '').replace(/"/g, '&quot;');
                    const checkConcluido = (prod.conferido || (declarado > 0 && qtdFeita >= declarado)) ? 'checked' : '';
                    const classeLinha = (prod.conferido || qtdFeita >= declarado) ? 'item-row item-ok' : 'item-row';

                    if (declarado > 0 && qtdFeita >= declarado) feitos++;
                    else if (qtdFeita > 0) parciais++;
                    else naoFeitos++;
                    if (!prod.status_controle || /^pendente$/i.test(statusCtrl)) semMarcar++;

                    itensHtml += `
                        <div class="${classeLinha}">
                            <div class="item-main-col">
                                <input type="checkbox" ${checkConcluido} onchange="window.marcarItemConferido('${item.id_envio}', '${sku}', this.checked)">
                                <div class="item-texts">
                                    <div class="item-sku">[${sku}]</div>
                                    <div class="item-title">${prod.titulo || ''}</div>
                                    <div class="item-sub">${prod.item_id || prod.inventory_id || ''}</div>
                                </div>
                            </div>
                            <div class="item-declarado-col">${declarado} un</div>
                            <div>
                                <input class="item-input" type="number" min="0" value="${qtdFeita}" onchange="window.atualizarQtdFeitaItem('${item.id_envio}', '${sku}', this.value)">
                            </div>
                            <div>
                                <select class="item-select" onchange="window.salvarStatusControleItem('${item.id_envio}', '${sku}', this.value)">
                                    <option value="Pendente" ${statusCtrl === 'Pendente' ? 'selected' : ''}>Pendente</option>
                                    <option value="Parcial" ${statusCtrl === 'Parcial' ? 'selected' : ''}>Parcial</option>
                                    <option value="Feito" ${statusCtrl === 'Feito' ? 'selected' : ''}>Feito</option>
                                    <option value="Não Feito" ${statusCtrl === 'Não Feito' ? 'selected' : ''}>Não Feito</option>
                                </select>
                            </div>
                            <div>
                                <input class="item-input" type="text" placeholder="Ex: caixa avariada" value="${observacaoItem}" onblur="window.salvarObsItem('${item.id_envio}', '${sku}', this.value)">
                            </div>
                        </div>`;
                });
                ...
                resumoItensHtml = `
                    <div class="itens-resumo-bar">
                        <strong>Controle dos itens:</strong>
                        <span>${feitos}/${entriesItens.length} feitos</span>
                        <span>• ${parciais} parciais</span>
                        <span>• ${naoFeitos} não feitos</span>
                        <span>• ${semMarcar} sem marcar</span>
                    </div>`;
            }
            ...
            trDetalhe.innerHTML = `
                <td colspan="8" style="padding:12px;">
                    ...
                    ${itensHtml ? `<div style="margin-bottom:8px;">${resumoItensHtml}${itensHtml}</div>` : `<div style="font-size:12px; color:#9aa4b2; margin-bottom:8px;">Nenhuma composição de itens registrada.</div>`}
