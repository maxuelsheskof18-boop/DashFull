// modulo.vesco-v8-operacional.js — V10.14 LINHA AO VIVO MULTIPONTOS
// Correções: Flex sem ERP, logística sem entregues, faturamento mensal com seletor de mês, coordenadas sem inversão.

(function(){
  if (window.VescoV8 && window.VescoV8.__v1014) return;
  ...
  async function saveFirebaseSnapshot(snap){
    ...
    try{ await firebasePut(firebaseCacheKey(), snap, 6500); }catch(e){ console.warn("V10.14: não salvou cache por data no Firebase.", e.message||e); }
    try{ await firebasePut("vesco_cache/painel/latest", snap, 6500); }catch(e){ console.warn("V10.14: não salvou cache latest no Firebase.", e.message||e); }
  }
  async function firebasePatchOrder(id, patch){
    ...
    catch(e){ console.warn("V10.14: patch Firebase falhou.", e.message||e); }
  ...
  async function refreshFromAppsScriptBackground(){
    ...
    try{
      ...
      console.warn("V10.14: Apps Script ERP lento/indisponível; Firebase mantém UI.", e.message);
    }
    ...
    try{
      ...
      console.warn("V10.14: Apps Script Flex lento/indisponível; Firebase mantém UI.", e.message);
    }
    ...
    try{
      ...
      console.warn("V10.14: rotas Apps Script lento/indisponível; Firebase/local mantém UI.", e.message);
    }
...
async function loadData(force=false){
    ...
    // V10.14: botão Atualizar força Apps Script e regrava o cache,
    // sem usar snapshot antigo do Firebase/localStorage.
    if(force){
      try{
        await refreshFromAppsScriptBackground();
      }catch(e){
        console.warn("V10.14: atualização forçada falhou.", e);
      }
      state.loading=false;
      showLoading(false);
      try{ render(); }catch(e){}
    ...
    // V10.14: Firebase-first. A tela não fica presa esperando Apps Script.
...
function layout(){
  ...
  function separacaoList(){
    return dedup(state.orders)
      ...
      .filter(o=>{
        const s=norm(statusAll(o));
        // Em aberto é pedido que ainda precisa entrar no fluxo de separação.
        // Não usa data prevista de entrega para ocultar.
        return !s ||
...
function fastRouteLink(rota, opts={}){
    ...
    // V10.14: SEMPRE leva data como fallback.
  ...
  function routeOrdersRows(){
    ...
    rotas.forEach(r=>{
      ...
      stops.forEach((stop,idx)=>{
        ...
        // V10.14: aplica entregas salvas no Firebase dentro da própria rota.
  ...
  async function refreshMotoristasLocalizacao(renderDom=true){
    try{
      ...
      console.warn("V10.14: localização motorista indisponível.", e.message||e);
      return state.motoristasLocalizacao||{};
    }
  }
  ...
  async function attachDriverRealtimeListener(){
    ...
    try{
      ...
      console.log("V10.14: listener realtime de motoristas ativo.");
    }catch(e){
      console.warn("V10.14: listener realtime não iniciou; usando polling.", e.message||e);
    }
  }
  ...
  function renderMotoristasAoVivo(){
    ...
    return `<div class="v106-driver-grid">${rows.map(row=>{
      try{
        ...
        console.warn("V10.14: falha ao renderizar card do motorista; card ignorado.", e);
        return "";
      }
    }).join("")}</div>`;
  }
  ...
  function safeRenderMotoristasAoVivo(){
    try{
      ...
      console.warn("V10.14: Motoristas ao vivo não bloqueou o painel.", e);
...
function renderDriverLiveMap(forceFit=false){
  ...
  async function shareRouteById(id){
    ...
    const ok=await copyRouteLink(link);
    if(ok) console.log("V10.14: link copiado automaticamente.");
  }
  ...
  async function updateStatus(id,statusNovo){
    // V10.14: status instantâneo no Firebase com operador e horário real de separação.
    try{
...
function render(){
    try{
      ...
      console.error("V10.14: render seguro capturou erro.", e);
      showLoading(false);
